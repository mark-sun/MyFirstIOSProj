...
class RatingControl: UIView {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        for _ in 0..<starCount {
            let button = UIButton()
            button.setImage(whiteStartImage, forState: .Normal)
            button.setImage(blackStartImage, forState: .Selected)
            button.setImage(blackStartImage, forState: [.Highlighted, .Selected])
